...
**All colors MUST be defined in ONE place**: `src/styles/global.css` in the `:root` block (lines 40-100).
...
**Exception**: Context-aware color overrides in specific components (e.g., `shiki-hap-theme.css`) MAY use local custom properties, but still in `hsl()` format:
...
1. **Define in `:root` first** (src/styles/global.css lines 40-100):
...
- [ ] All color definitions in `src/styles/global.css` `:root` block
...
**Complete HAP color system** (see `src/styles/global.css:40-100` for actual values):
...
- **Single source of truth** - All colors in `src/styles/global.css` `:root` block
