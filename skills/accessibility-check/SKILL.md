...
> **Single source of truth**: All color values are defined in `src/styles/global.css` using `hsl()` format with alpha channel where needed. See `:root` variables (lines 40-100) for actual color values.
