...
<div class="demo-container">
  ...
  <header style="text-align: center; margin-bottom: 2rem;">
    <h1 style="color: hsl(32 62% 47%); margin-bottom: 0.5rem;">HAP's [Demo Name]</h1>
    <p style="font-size: 1.125rem; color: var(--dark-brown);">[Educational tagline]</p>
  </header>
  ...
  <section class="output-section" aria-labelledby="output-heading">
    ...
    <button id="copy-btn" class="demo-copy-btn" aria-label="Copy code to clipboard">
      Copy Code
    </button>
  </section>
...
```html
<button id="copy-btn" class="demo-copy-btn" aria-label="Copy code to clipboard">Copy Code</button>
```
...
window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", (e) => {
  updateSystemStatus();
  announce(e.matches ? "System switched to dark mode" : "System switched to light mode");
});
```
...
```html
<nav class="page-navigation" aria-label="Demo navigation"></nav>
```
...
```html
<div style="background: hsl(32, 76%, 63%); padding: 20px; margin: 10px;">Content</div>
```
...
```html
<button onclick="copyCode()">Copy</button> <a href="#section">Navigate here</a>
```
